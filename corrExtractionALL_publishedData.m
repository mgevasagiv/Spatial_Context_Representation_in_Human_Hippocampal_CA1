...
% subject specific onset files are extracted using stimuliExtraction.m
onsetFiles_root = 'E:\spatcon_hdz\data_p\onsetFiles\';
analyzed_mri_dir = 'E:\spatcon_hdz\data_p\analyzed_mri_mixed_items\';
...
for subj_i = 1:length(subjects)
    ...
    for hem_i = 1:2
        ...
        for roi_i = 1:length(rois_of_most_interest)
            ...
            fileName = fullfile(analyzed_mri_dir,sprintf('s%03d',subj),'ROIs',hemi_label,...
                sprintf('br%s_pattern_corr_no_outlier_trials_all_runs.mat',roi_str));
            fileNameIDs = fullfile(analyzed_mri_dir,sprintf('s%03d',subj),'ROIs',hemi_label,...
...
filename = 'pattern_corr_all_zsc_subjects';
save('E:\spatcon_hdz\data_p\corrAnalysis\pattern_corr_zscored_all_subjects.mat','object_info_all_rois','pattern_corr_all_rois')
...
for subj_i = 1:length(object_info_all_rois)
    ...
    title_f = sprintf('s%03d_rawR_leftH_all_ROIs',subj_i);
    ...
    close(f0)
     ...
     title_f = sprintf('s%03d_fisherZ_R_leftH_all_ROIs',subj_i);
    f0 = newA4figure(title_f);
    for roi_i = 1:length(rois_of_most_interest)
